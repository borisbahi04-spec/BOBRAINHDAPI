...
private async obtainErpTokenIfNeeded() {
  ...
  if (!this.ERP_LOGIN || !this.ERP_PASSWORD) {
   this.erpToken= await axios.post(`http://10.0.0.52:3335/posgpt-backend/api/v1/auth/login`, {
      username:  'admin',
      password:  'admin',
  ...
  try {
    const resp = await axios.post(`http://10.0.0.52:3335/posgpt-backend/api/v1/auth/login`, {
     username:  'admin',
      password: 'admin',
    }, { timeout: 5000 });
...
private async connectToErp() {
  try {
    ...
    this.erpSocket = io(`http://10.0.0.52:3335/ws`, {
